...
function FillTooltip({ active, payload }: any) {
  ...
  return (
    <div className="k-tooltip">
      ...
      <div className="k-tooltip__row"><span className="k-tooltip__k">Slippage</span><span>{"\u2212"}${slip.toFixed(2)}</span></div>
    </div>
  );
}

export function Proof({ data }: { data: LandingData }) {
  const rows = (data.fills ?? []).map((f) => ({
    ...f,
    gap: Math.max(0, f.expected - f.realized),
  }));
  const hasFills = rows.length > 0;
  const s = data.stats;
          ...
          <div className="k-chart" style={{ height: 300 }} aria-hidden="true">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={rows} margin={{ top: 12, right: 16, bottom: 4, left: 8 }}>
                ...
                <Line type="monotone" dataKey="expected" stroke={ACCENT} strokeWidth={1.6} strokeDasharray="5 4" dot={false} isAnimationActive={false} />
                <Line type="monotone" dataKey="realized" stroke={ACCENT} strokeWidth={2} dot={false} isAnimationActive={false} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
